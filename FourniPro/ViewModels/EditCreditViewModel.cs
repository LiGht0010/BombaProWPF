...
/// <summary>Edit-dialog ViewModel for an existing Credit.</summary>
public class EditCreditViewModel : ObservableObject
{
    ...
    private string _numeroCredit = string.Empty;
    public string NumeroCredit
    {
        get => _numeroCredit;
        set => SetProperty(ref _numeroCredit, value);
    }
    ...
    public ProduitDto? SelectedProduit
    {
        ...
        set => SetProperty(ref _selectedProduit, value);   // no price auto-overwrite in edit mode
    }
    ...
    public string? ChequeReference
    {
        ...
        set => SetProperty(ref _chequeReference, value);
    }
    ...
    private bool _hasCheque;
    /// <summary>Drives cheque-field visibility in the dialog.</summary>
    public bool HasCheque
    {
        get => _hasCheque;
        set => SetProperty(ref _hasCheque, value);
    }
    ...
    public EditCreditViewModel(CreditDto credit)
    {
        _creditId             = credit.CreditId;
        _originalAjoutePar    = credit.AjoutePar;
        _originalDateCreation = credit.DateCreation;
        _pendingClientId      = credit.ClientID;
        _pendingProduitId     = credit.ProduitID;
        _pendingEmployeId     = credit.EmployeId;

        NumeroCredit          = string.IsNullOrWhiteSpace(credit.NumeroCredit)
                                    ? GenerateNumero()
                                    : credit.NumeroCredit;
        DateCredit            = credit.DateCredit.ToDateTime(TimeOnly.MinValue);
        Quantite              = credit.Quantite;
        PrixUnitaire          = credit.PrixUnitaire;
        Remise                = credit.Remise;
        MontantTotal          = credit.MontantTotal;
        SelectedStatut        = credit.Statut;
        ChequeReference       = credit.ChequeReference;
        SelectedStatutCheque  = credit.StatutCheque;
        _hasCheque            = !string.IsNullOrWhiteSpace(credit.ChequeReference);
        Note                  = credit.Note;
        Reference             = credit.Reference;
        ReferenceFile         = credit.ReferenceFile;
    ...
    private async Task SaveAsync()
    {
        ...
        {
            ErrorMessage = LanguageManager.Instance["EditCreditValidationClient"];
            return;
        }
        if (SelectedProduit is null)
        {
            ErrorMessage = LanguageManager.Instance["EditCreditValidationProduit"];
            return;
        }
        if (Quantite is null or <= 0)
        {
            ErrorMessage = LanguageManager.Instance["EditCreditValidationQuantite"];
            return;
        }
        ...
        try
        {
            var dto = new CreditDto
            {
                ...
                ChequeReference    = HasCheque ? NullIfBlank(ChequeReference) : null,
                StatutCheque       = HasCheque ? NullIfBlank(SelectedStatutCheque) : null,
    ...
    {
        var n = DateTime.Now;
        return $"C-{n:yyyy-MM-dd-HH-mm-ss}";
    }
}
