...
public class DialogService : IDialogService
{
    ...
    public async Task<BatchAllocationResponseDto?> ShowAchatAllocationPopupForNewAchatAsync(AchatDto achat)
    {
        ...
        // Require password confirmation before showing allocation popup
        var passwordConfirmed = await ShowPasswordConfirmationPopupAsync();
    ...
    public async Task<bool> ShowPasswordConfirmationPopupAsync()
    {
        var popup = new PasswordConfirmationPopup();
        var result = await CurrentPage.ShowPopupAsync(popup);
        return result is bool confirmed && confirmed;
    }

    // ???????????????????????????????????????????????????????????????????????
    // STOCK WITHDRAWAL POPUPS
    // ???????????????????????????????????????????????????????????????????????

    public async Task<StockWithdrawalResponseDto?> ShowStockWithdrawalCreatePopupAsync()
    {
        var popup = new StockWithdrawalCreatePopup();
        var result = await CurrentPage.ShowPopupAsync(popup);
        return result as StockWithdrawalResponseDto;
    }
}
