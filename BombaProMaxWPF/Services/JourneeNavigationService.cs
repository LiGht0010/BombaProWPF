...
/// In WPF, navigation is handled via NavigationRequested event instead of Shell.
/// </summary>
public partial class JourneeNavigationService : ObservableObject
{
    /// <summary>
    /// Raised when navigation to a route is requested. WPF host subscribes to handle navigation.
    /// </summary>
    ...
    public async Task EndJourneeAsync()
    {
        ...
        // Request navigation to HomePage
    ...
    private Task NavigateToCurrentStepAsync()
    {
        if (CurrentStepIndex < 0 || CurrentStepIndex >= _journeeRoutes.Count) return Task.CompletedTask;
